...
function () {
  function _default() {
    ...
    this.productCount = 0;
    this.addFlashSaleProducts(GreensIntl.data['flash_sale.products']);

    if (this.productCount === 0) {
      this.addProduct();
    }

    this.addFlashSaleProductsError(GreensIntl.errors['flash_sale.products']);
...
function () {
  ...
  _createClass(_default, [{
    ...
    value: function normalizeAttributes(product) {
      ...
      if (!$.isEmptyObject(GreensIntl.errors['flash_sale.products'])) {
        return {
          id: product.id,
          name: product.name,
          pivot: {
