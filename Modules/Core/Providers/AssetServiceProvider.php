...
use Modules\Core\Foundation\Asset\Manager\GreensIntlAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\GreensIntlAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, GreensIntlAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new GreensIntlAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (! is_null($assets = config("greensintl.themes.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("greensintl.modules.{$module->getAlias()}.assets");
